...
export const generateQuestions = (key: keyof typeof items) => {
  ...
  const trueOrFalseQuestions = generateTrueOrFalseQuestions(cards, numberOfTrueOrFalseQ);
  const multipleOptionQuestions = generateMultipleOptionQuestion(cards, numberOfMultipleOptionQ);
  const openEndedQuestions = generateOpenEndedQuestion();

  return [...trueOrFalseQuestions, ...multipleOptionQuestions, ...openEndedQuestions];
}
